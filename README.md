...
1. **Download** `intervals-icu-jan-1.3.2.mcpb` from the [latest release](https://github.com/jancrab/intervals-mcp-server/releases/latest) on GitHub.
...
- **`link_activity_to_event` returns `status: "draft_unrecoverable"`.** The activity is in a pre-normalization state too deep for intervals.icu's link endpoint to resolve. This typically affects activities uploaded by Zwift's built-in tests (FTP, ramp, stress), which appear to upload via the Zwift→Strava→intervals.icu path and bypass intervals.icu's normalization. The link endpoint requires the activity to already be in intervals.icu's canonical ID space (`i…`-prefixed); pre-normalization activities still hold their upstream raw integer ID. Remediation: open the activity in intervals.icu's web UI (`https://intervals.icu/activities/{id}`), give it a name, and save. The web UI bypasses the API path and forces normalization. After save, the API will expose full power/HR/duration data on subsequent calls. Workaround: prefer running prescribed `.zwo` workouts over Zwift's built-in tests when test data needs to flow through the MCP automatically. (Status added in v1.3.2.)
